...
//
// If MODORE_IPC_SOCKET is set to a filesystem path, the listener and --trigger use only that path
// (e2e / multiple hosts). Otherwise the socket is $XDG_RUNTIME_DIR/modore.sock (see ipc.cpp).
...
// Fills `out` with the socket path; returns bytes written (excluding NUL) or 0 on error.

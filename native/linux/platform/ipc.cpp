...
std::size_t ipc_socket_path(char *out, std::size_t cap) {
  ...
  if (rt && *rt) {
    ...
    // Matches systemd user session (%t = /run/user/$UID). Hyprland keybind
    // `exec` often drops XDG_RUNTIME_DIR; /tmp/modore.$uid.sock then never
    // reached the real listener.
    std::snprintf(out, cap, "/run/user/%d/modore.sock",
...
static void pickup_connect_candidates(std::vector<std::string> *out) {
  ...
  char legacy[256];
  std::snprintf(legacy, sizeof(legacy), "/tmp/modore.%d.sock",
                static_cast<int>(getuid()));
...
int ipc_send_pickup() {
  ...
  for (const std::string &path : paths) {
    ...
    close(fd);
    MODORE_E2E_LOGF("ipc_send_pickup: connected + wrote pickup -> %s",
                    path.c_str());
    modore_log("ipc",
               "trigger: sent pickup to %s (if conversion does nothing, see "
               "host log for "
  ...
  modore_log("ipc",
             "trigger: connect failed for all tried paths [%s] — is "
             "modore-host running? "
...
void ipc_start_background(std::function<void()> on_pickup) {
  std::thread([cb = std::move(on_pickup)]() mutable {
    char path[256]{};
    ...
    if (bind(srv, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) < 0) {
      modore_log("ipc",
                 "bind(%s) failed — another modore-host may be running, or "
                 "stale socket; "
    ...
    for (;;) {
      ...
      close(c);
      if (n <= 0) {
        modore_log("ipc", "accept closed before data (n=%zd)",
      ...
      if (std::strncmp(buf, "pickup", 6) == 0) {
        ...
        modore_log("ipc", "ignored non-pickup message (%.*s)",

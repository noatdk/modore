...
int log_trampoline(void * /*userdata*/, int level, const char *tag,
                   const char *msg) {
  const char *prefix = "";
  switch (level) {
  case MDR_LOG_WARN:
  ...
  std::snprintf(composite_tag, sizeof(composite_tag), "scripting:%s",
                tag ? tag : "lua");
...
int default_pickup(void * /*userdata*/, const mdr_pickup_ctx_t *ctx,
                   mdr_span_t *out) {
  if (!ctx || !out || !ctx->full_text)
    return 0;
...
int default_replacement(void * /*userdata*/, const char * /*app_id*/,
                        const mdr_span_t * /*span*/, const char *const *cands,
                        std::size_t n_cands, char *out_buf, std::size_t out_cap,
                        std::size_t *out_len) {
  if (!cands || n_cands == 0 || !cands[0] || !out_buf || !out_len ||
      out_cap == 0)
    return 0;
...
int default_route(void * /*userdata*/, const char * /*app_id*/,
...
void boot(const std::string &script_dir) {
  ...
  mdr_set_defaults(g_engine, nullptr, default_pickup, default_replacement,
                   default_route);
  if (mdr_load_dir(g_engine, script_dir.c_str()) != 0) {
    modore_log("scripting", "script dir load failed (dir=%s)",
               script_dir.c_str());
  } else {
    modore_log("scripting", "script dir loaded (dir=%s)", script_dir.c_str());
  }
  modore_log("scripting", "engine ABI v%d loaded (dir=%s)", mdr_abi_version(),
             script_dir.c_str());
}
...
std::optional<Route> route_for(const char *app_id) {
  ...
  case MDR_ROUTE_AX:
    return Route::Ax;
  case MDR_ROUTE_KEYSTROKE:
    return Route::Keystroke;
  case MDR_ROUTE_CLIPBOARD:
...
replacement(const char *app_id, std::size_t span_start_byte,
  ...
  int rc = mdr_replacement(g_engine, app_id, &span,
                           raw.empty() ? nullptr : raw.data(), raw.size(), buf,
                           sizeof(buf), &out_len);
  if (rc != 1 || out_len == 0)
    return std::nullopt;
...
candidates(const char *app_id, const std::vector<std::string> &list,
  ...
  int rc =
      mdr_candidates(g_engine, app_id, raw.empty() ? nullptr : raw.data(),
                     raw.size(), current_index, buf, sizeof(buf), &out_count);
  if (rc != 1 || out_count == 0)
    return std::nullopt;

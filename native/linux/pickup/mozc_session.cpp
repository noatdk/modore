...
void mozc_prefetch_candidates_async(const std::string &romaji,
  ...
  std::thread([romaji, committed]() {
    auto full = mozc_convert_utf8_with_candidates(romaji);
    if (!full) {
      modore_log("session", "candidate prefetch failed for '%s'",
                 romaji.c_str());
      return;
    }
    std::lock_guard<std::mutex> lock(g_conversion_session_mu);
    if (!g_has_conversion_session ||
        g_conversion_session.source_romaji != romaji) {
      return;
    }
    int idx = g_conversion_session.candidate_index;
    g_conversion_session.candidates = normalize_candidate_session_state(
        committed, std::move(full->second), &idx);
    g_conversion_session.candidate_index = idx;
    modore_log("session", "candidate prefetch ready: %zu for '%s'",
               g_conversion_session.candidates.size(), romaji.c_str());
  }).detach();
}
...
void set_conversion_session(ConversionSession session) {
  ...
  std::string committed;
  {
    ...
    romaji = g_conversion_session.source_romaji;
    committed = g_conversion_session.current_text;
  }
  if (!romaji.empty() && mozc_uses_atzc_backend()) {
    mozc_prefetch_candidates_async(romaji, committed);
  }
}
